


import streamlit as st
import matplotlib.pyplot as plt
import plotly.express as px
import pandas as pd
import matplotlib.pyplot as plt
import plotly.graph_objs as go

st.write("""
# 823 HW4
##### by Candice Li
""")




# Figure 1
data1 = pd.read_excel('field_of_study.xlsx')

data1=data1.iloc[[2,29,70,89,101,108,118,124,142,161,185,190,194,195,196,125,224,248,254,
                  268,273,292, 296, 309, 319,331,350,366,373, 382
                  ]]

data1=data1.rename(columns={"Fine field of study":"field", 2008:"yr2008",
                      2009:"yr2009", 2010:"yr2010", 2011:"yr2011",
                      2012:"yr2012", 2013:"yr2013", 2014:"yr2014",
                      2015:"yr2015", 2016:"yr2016", 2017:"yr2017"})


selected_metrics = st.selectbox(
    label="Choose Year", options=['2008','2009','2010',
                                '2011', '2012', '2013',
                                '2014', '2015', '2016',
                                '2017']
)

fig1 = go.Figure()

if selected_metrics == '2008':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2008"],
                    name='2008'))

if selected_metrics == '2009':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2009"],
                    name='2009'))

if selected_metrics == '2010':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2010"],
                    name='2010'))

if selected_metrics == '2011':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2011"],
                    name='2011'))

if selected_metrics == '2012':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2012"],
                    name='2012'))

if selected_metrics == '2013':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2013"],
                    name='2013'))

if selected_metrics == '2014':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2014"],
                    name='2014'))

if selected_metrics == '2015':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2015"],
                    name='2015'))

if selected_metrics == '2016':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2016"],
                    name='2016'))

if selected_metrics == '2017':
	fig1.add_trace(go.Bar(x=data1.field, y=data1["yr2017"],
                    name='2017'))

fig1.update_layout(
    title={'text': 'Fig 1. Number of  doctorate recipients by Field of study in selected year',
           'xanchor': 'center',
           'yanchor': 'top',
           'x': 0.5})
st.plotly_chart(fig1, use_container_width=True)




# Figure 2
data2 = pd.read_excel('location.xlsx',  skiprows=3)
code = {'Alabama': 'AL',
        'Alaska': 'AK',
        'Arizona': 'AZ',
        'Arkansas': 'AR',
        'California': 'CA',
        'Colorado': 'CO',
        'Connecticut': 'CT',
        'Delaware': 'DE',
        'District of Columbia': 'DC',
        'Florida': 'FL',
        'Georgia': 'GA',
        'Hawaii': 'HI',
        'Idaho': 'ID',
        'Illinois': 'IL',
        'Indiana': 'IN',
        'Iowa': 'IA',
        'Kansas': 'KS',
        'Kentucky': 'KY',
        'Louisiana': 'LA',
        'Maine': 'ME',
        'Maryland': 'MD',
        'Massachusetts': 'MA',
        'Michigan': 'MI',
        'Minnesota': 'MN',
        'Mississippi': 'MS',
        'Missouri': 'MO',
        'Montana': 'MT',
        'Nebraska': 'NE',
        'Nevada': 'NV',
        'New Hampshire': 'NH',
        'New Jersey': 'NJ',
        'New Mexico': 'NM',
        'New York': 'NY',
        'North Carolina': 'NC',
        'North Dakota': 'ND',
        'Ohio': 'OH',
        'Oklahoma': 'OK',
        'Oregon': 'OR',
        'Pennsylvania': 'PA',
        'Rhode Island': 'RI',
        'South Carolina': 'SC',
        'South Dakota': 'SD',
        'Tennessee': 'TN',
        'Texas': 'TX',
        'Utah': 'UT',
        'Vermont': 'VT',
        'Virginia': 'VA',
        'Washington': 'WA',
        'West Virginia': 'WV',
        'Wisconsin': 'WI',
        'Wyoming': 'WY'}
data2['Code'] = data2['State or location'].map(code)
fig2 = px.choropleth(data2,
                    locations='Code',
                    color='Doctorate recipients',
                    color_continuous_scale='spectral_r',
                    hover_name='State or location',
                    locationmode='USA-states',
                    scope='usa')

fig2.add_scattergeo(
    locations=data2['Code'],
    locationmode='USA-states',
    text=data2['Code'],
    mode='text')

fig2.update_layout(
    title={'text':'Fig 2. Number of doctorate recipients by State: 2017',
           'xanchor':'center',
           'yanchor':'top',
           'x':0.5})

st.plotly_chart(fig2, use_container_width=True)